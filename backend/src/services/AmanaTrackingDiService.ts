/**
 * Parallel AMANA tracking path via DataImpulse (Morocco residential).
 * Intentionally separate from AmanaTrackingService (Turnoxy) so both can be compared.
 */
...
} from './AmanaTrackingService';

export { AmanaTrackingError, amanaTrackingErrorToHttp };
...
/** Production VPS: true. Local Morocco: set AMANA_DI_PROXY_ENABLED=false */
const AMANA_DI_PROXY_ENABLED = true;

/** DataImpulse residential MA — override with AMANA_DI_PROXY_URL in .env */
const HARDCODED_AMANA_DI_PROXY_URL =
  'http://fa6daed2aa67067d6df3__cr.ma:2bac4f7eea9a4e7a@gw.dataimpulse.com:823';

function isDiProxyEnabled(): boolean {
  const env = process.env.AMANA_DI_PROXY_ENABLED?.trim().toLowerCase();
  if (env === '0' || env === 'false' || env === 'off') return false;
  if (env === '1' || env === 'true' || env === 'on') return true;
  return AMANA_DI_PROXY_ENABLED;
}

function getDiProxyUrl(): string | undefined {
  if (!isDiProxyEnabled()) return undefined;
  return process.env.AMANA_DI_PROXY_URL?.trim() || HARDCODED_AMANA_DI_PROXY_URL;
}
...
function redactProxyForLog(proxyUrl: string): string {
  try {
    ...
    return '[invalid AMANA_DI_PROXY_URL]';
  }
}
...
function openHttpProxyTunnel(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (proxy.protocol !== 'http:' && proxy.protocol !== 'https:') {
      reject(
        new Error(
          `Unsupported AMANA_DI_PROXY_URL protocol "${proxy.protocol}". Use http://USER:PASS@HOST:PORT.`
        )
      );
      return;
    }
    ...
    req.on('connect', (res, socket) => {
      if ((res.statusCode || 0) !== 200) {
        ...
        const hint =
          code === 407 ? ' (bad user/pass — check DataImpulse credentials)' : '';
        finish(() =>
...
export type AmanaTrackingDiPayload = AmanaTrackingPayload & {
...
type CacheEntry = {
  expiresAt: number;
  data: AmanaTrackingDiPayload;
};

const diTrackingCache = new Map<string, CacheEntry>();

function cacheKey(trackingCode: string): string {
  return `di:${trackingCode.trim().toUpperCase()}`;
}
...
export function httpGetTextDi(urlString: string, timeoutMs: number): Promise<HttpGetResult> {
  const maxRedirects = 5;
  const proxyUrl = getDiProxyUrl();

  if (proxyUrl) {
    console.info('[amana-tracking-di] proxy configured', redactProxyForLog(proxyUrl));
  }
        ...
        req.on('error', (err: any) => {
          ...
          if (/407/.test(message) || /Proxy Authentication Required/i.test(message)) {
            finish(() =>
              reject(
                new Error(
                  'Proxy authentication failed (407). Check DataImpulse credentials.'
                )
              )
            );
            return;
...
export function diTrackingErrorToHttp(
  error: unknown
  ...
  return amanaTrackingErrorToHttp(error);
}

export class AmanaTrackingDiService {
  constructor(
    private readonly prisma: PrismaClient,
    private readonly httpGet: typeof httpGetTextDi = httpGetTextDi
  ) {}

  clearCache(trackingCode?: string) {
    if (trackingCode) {
      diTrackingCache.delete(cacheKey(trackingCode));
      return;
    }
    diTrackingCache.clear();
  }
  ...
  async getForOrder(
  ...
  ): Promise<AmanaTrackingDiPayload> {
    ...
    if (!options.refresh) {
      const hit = diTrackingCache.get(key);
      if (hit && hit.expiresAt > Date.now()) {
        return { ...hit.data, cached: true };
      }
    } else {
      diTrackingCache.delete(key);
    }

    const data = await this.fetchAndParse(trackingCode);
    diTrackingCache.set(key, { expiresAt: Date.now() + CACHE_TTL_MS, data });
    return data;
  }

  private async fetchAndParse(trackingCode: string): Promise<AmanaTrackingDiPayload> {
    ...
    try {
      if (proxyUrl) {
        console.info('[amana-tracking-di] fetching via proxy', redactProxyForLog(proxyUrl));
      } else {
        console.info('[amana-tracking-di] fetching direct (proxy disabled)');
      }
      ...
      console.error('[amana-tracking-di] upstream request failed', {
        url,
      ...
      if (err?.name === 'AbortError' || /timeout/i.test(detail)) {
        throw new AmanaTrackingError(
          'AMANA tracking request timed out (DataImpulse)',
          'UPSTREAM_TIMEOUT',
          detail
        );
      }
      throw new AmanaTrackingError(
        'Unable to reach AMANA tracking via DataImpulse proxy',
        'UPSTREAM_ERROR',
        detail
      );
    }
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      console.error('[amana-tracking-di] upstream bad status', {
      ...
      if (response.statusCode === 407) {
        throw new AmanaTrackingError(
          'DataImpulse proxy rejected username/password (HTTP 407). Check AMANA_DI_PROXY_URL.',
          'UPSTREAM_ERROR',
    ...
    try {
      ...
      console.error('[amana-tracking-di] invalid JSON/HTML body', err?.message);
      throw new AmanaTrackingError(
    ...
    try {
      ...
      console.error('[amana-tracking-di] HTML parse failed', err?.message);
      throw new AmanaTrackingError(
