            ...
            <TableBody>
              {variantHistory.map((item: any) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    <Badge className={getChangeTypeColor(item.type, item.quantity)} variant="outline">
                  ...
                  <TableCell className="text-xs text-gray-500 max-w-[150px] truncate" title={item.reason}>
                    {item.reason}
                  </TableCell>
                </TableRow>
              ))}
