import { CheckCircle2, FlaskConical, Loader2, RotateCw } from "lucide-react";
...
type AmanaTrackingDiSheetProps = {
  open: boolean;
...
function SummaryRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="min-w-0">
      <p className="text-[11px] font-medium uppercase tracking-wide text-teal-700/70">{label}</p>
...
function TrackingSkeleton() {
  return (
    <div className="space-y-4 animate-pulse" data-testid="amana-tracking-di-skeleton">
      <div className="space-y-2 rounded-lg border border-teal-100 bg-teal-50/40 p-3">
        <div className="h-3 w-24 rounded bg-teal-200/70" />
        <div className="h-6 w-40 rounded bg-teal-200/70" />
        <div className="h-3 w-32 rounded bg-teal-200/70" />
        <div className="h-4 w-full rounded bg-teal-200/70" />
      </div>
      <div className="space-y-3">
        {[0, 1, 2].map((i) => (
          <div key={i} className="flex gap-3">
            <div className="mt-1 h-2.5 w-2.5 rounded-full bg-teal-200" />
...
function TrackingTimeline({ data }: { data: AmanaTrackingDiData }) {
  return (
    <ol className="relative space-y-0" data-testid="amana-tracking-di-timeline">
        ...
        return (
          ...
          >
            <div className="relative flex justify-center pt-1.5" aria-hidden>
              {!isLast && (
                <span className="absolute top-3 bottom-[-16px] w-px bg-teal-200" />
              )}
              <span
                className={cn(
            ...
            <div className={cn("min-w-0 space-y-1.5", isFirst && "rounded-md bg-teal-50/70 p-2 -ml-1")}>
...
export function AmanaTrackingDiSheet({
  open,
  onOpenChange,
  order,
  onOrderStatusPatched,
}: AmanaTrackingDiSheetProps) {
  const orderId = order?.id ?? null;
  const queryClient = useQueryClient();
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [refreshError, setRefreshError] = useState<string | null>(null);
  const query = useAmanaTrackingDi(orderId, open && Boolean(orderId));
  ...
  useEffect(() => {
    if (!open) {
      ...
      if (orderId) {
        void queryClient.cancelQueries({ queryKey: ["amana-tracking-di", "order", orderId] });
      }
    }
  ...
  const handleRefresh = async () => {
    ...
    setRefreshError(null);
    try {
      const { data: payload } = await api.get(`/amana-tracking-di/order/${orderId}`, {
        params: { refresh: 1 },
        timeout: 30000,
      });
      queryClient.setQueryData(["amana-tracking-di", "order", orderId], payload);
    } catch (error: any) {
      const message =
        error?.response?.data?.error ||
        (error?.code === "ECONNABORTED"
          ? "Request timed out — DataImpulse path could not reach AMANA in time"
          : "Unable to refresh AMANA tracking (DataImpulse)");
  ...
  const errorFromQuery =
    ...
    ((query.error as any)?.code === "ECONNABORTED"
      ? "Request timed out — DataImpulse path could not reach AMANA in time"
      : null) ||
    (query.isError ? "We couldn't retrieve tracking via DataImpulse." : null);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        className="flex w-full flex-col gap-0 overflow-hidden border-l-4 border-l-teal-500 p-0 sm:max-w-[440px]"
        data-testid="amana-tracking-di-sheet"
      >
        <SheetHeader className="shrink-0 space-y-0 border-b border-teal-100 bg-gradient-to-br from-teal-50 to-cyan-50/80 px-4 pb-3 pt-4 pr-12 text-left sm:px-5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center rounded-md border border-teal-300 bg-teal-600 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white">
                  DI TEST
                </span>
                {data && !query.isError ? (
                  <span className="inline-flex items-center gap-1 rounded-full border border-teal-200 bg-white/80 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-teal-800">
              ...
              <p
                className="font-mono text-sm font-semibold tabular-nums text-teal-800"
                data-testid="amana-tracking-di-code"
              >
                {trackingCode}
              </p>
              <p className="text-[11px] text-teal-700/80">
                {data?.proxyLabel || "Proxy test path — compare with the truck icon"}
              </p>
              <SheetDescription className="sr-only">
                AMANA shipment tracking via DataImpulse for order #{order?.id}
              </SheetDescription>
            </div>
            <Button
              type="button"
              variant="outline"
              size="sm"
              className="h-8 shrink-0 gap-1.5 border-teal-300 bg-white text-teal-800 hover:bg-teal-50"
              onClick={() => void handleRefresh()}
              disabled={isRefreshing || !orderId || showInitialLoading}
              aria-label="Refresh DataImpulse tracking"
              title="Refresh DataImpulse tracking"
            >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-3 sm:px-5">
          {showInitialLoading ? (
            <div className="space-y-3">
              <p className="text-center text-xs text-teal-700/80">
                Contacting AMANA via DataImpulse… (max ~30s)
              </p>
              <TrackingSkeleton />
            </div>
          ) : showError ? (
            <div className="space-y-3 py-6 text-center" data-testid="amana-tracking-di-error">
              ...
              <p className="mx-auto max-w-sm text-xs text-slate-400">
                This is the DataImpulse test path. The original truck icon still uses Turnoxy.
              </p>
              <Button
                type="button"
                variant="outline"
            ...
            <div className="space-y-4">
              <section className="space-y-3 rounded-lg border border-teal-100 bg-white p-3 shadow-sm shadow-teal-100/50">
                {data.product ? (
                ...
                {suggestedStatus && suggestedStatus !== currentOrderStatus ? (
                  <div
                    ...
                    data-testid="amana-di-apply-status"
                  >
                    <p
                      className={cn(
                        "text-sm font-medium",
              ...
              <section className="grid grid-cols-2 gap-3 rounded-lg border border-teal-100 bg-teal-50/40 p-3">
              ...
              <section>
                <h3 className="mb-3 text-sm font-semibold text-teal-900">Tracking history</h3>
