...
const Delivery = () => {
  ...
  const scopedServices = isSuivi && user?.deliveryServiceIds?.length
    ? deliveryServices?.filter((service) => user.deliveryServiceIds!.includes(service.id))
    : deliveryServices;

  const filteredServices = scopedServices?.filter(service => 
